...
import { Layers, GraduationCap, RefreshCw, ShieldQuestion, ShieldX, UserCog, Users, Wallet } from 'lucide-react';
...
// Mirrors DarwinBreakdown's shape exactly (see darwin-breakdown.tsx) -- the
// backend's /reports/darwin-full-roster-breakdown route is structurally
// identical to /reports/darwin-breakdown, just scoped to the full-roster
// fallback-matched population (inDarwin && inDarwinFullRoster) instead of
// the primary Instructors-department pass (inDarwin && !inDarwinFullRoster).
type DarwinFullRosterBreakdown = {
  total_full_roster_matched: number;
...
const BREAKDOWN_QUERY_KEY = ['reports', 'darwin-full-roster-breakdown'];

function useDarwinFullRosterBreakdown() {
  return useQuery<DarwinFullRosterBreakdown>({
    queryKey: BREAKDOWN_QUERY_KEY,
    queryFn: async () => {
      const response = await fetch('/api/reports/darwin-full-roster-breakdown');
...
export default function DarwinFullRosterBreakdownPage() {
  const queryClient = useQueryClient();
  const breakdownQuery = useDarwinFullRosterBreakdown();
  ...
  return <div className="mx-auto max-w-[1350px]">
    <PageIntro
      ...
      title="Darwin Full Roster Breakdown"
      description="Everyone Darwinbox only surfaced via the full, unfiltered company roster -- not the Instructors-department pass Darwin Breakdown covers -- typically because their department field isn't literally 'Instructors – ...' (Mentors is the confirmed real case)."
      action={<button type="button" data-testid="button-refresh-darwin-full-roster-breakdown" onClick={refresh} className="inline-flex items-center gap-2 self-start rounded-lg border border-border bg-card px-3.5 py-2.5 text-[12px] font-bold text-foreground transition-colors hover:bg-secondary lg:self-auto"><RefreshCw size={14} /> Refresh</button>}
    />
    ...
    {breakdownQuery.isError && <QueryError message="Darwin Full Roster breakdown is unavailable right now." />}

    {data && <div className="space-y-5 animate-rise">
      {/* Headline KPI row */}
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <TopStat label="Full roster matched" value={formatKpi(data.total_full_roster_matched)} meta="Darwinbox, matched only via the full company roster" icon={<Layers size={17} />} tone="navy" />
        <TopStat label="Actual instructors" value={formatKpi(data.instructors.count)} meta={`${pct(data.instructors.count, data.total_full_roster_matched)} of these matches`} icon={<GraduationCap size={17} />} tone="teal" />
        <TopStat label="Mentors" value={formatKpi(data.mentors.count)} meta={`${pct(data.mentors.count, data.total_full_roster_matched)} of these matches`} icon={<Users size={17} />} tone="indigo" />
        <TopStat label="Others" value={formatKpi(data.others.total)} meta={`${pct(data.others.total, data.total_full_roster_matched)} of these matches`} icon={<ShieldQuestion size={17} />} tone="saffron" alert />
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5 shadow-xs sm:p-6">
        <div className="mb-5 flex items-start justify-between">
          <div>
            ...
            <h2 className="mt-1 text-[16px] font-extrabold tracking-[-0.03em]">Who else the full roster carries</h2>
          </div>
        </div>
      ...
      <BucketPanel
        ...
        subtitle="Full-roster-matched people with a real teaching designation, bucketed tech or non-tech"
        icon={<Users size={18} />}
      ...
      <BucketPanel
        title="Excluded"
        subtitle="Individually reviewed and confirmed not to be a teaching/instructor role, despite this full-roster match"
      ...
      <BucketPanel
        ...
        subtitle="Flagged payroll-converted despite also having a Darwin full-roster match -- worth a manual look"
        icon={<Wallet size={18} />}
      ...
      <BucketPanel
        title="Uncategorized"
        subtitle="Matched via the full roster, but didn't land in any bucket above -- needs a manual look"
      ...
      {!data.total_full_roster_matched && <EmptyState title="No full-roster matches yet" description="Sync Darwinbox (or upload a full-roster CSV) to see the fallback-match breakdown here." />}
    </div>}
  </div>;
}
