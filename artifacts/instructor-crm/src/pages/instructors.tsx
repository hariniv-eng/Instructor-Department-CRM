...
import { Filter, Mail, Plus, Search, SlidersHorizontal, UserRound, X } from 'lucide-react';
...
export default function InstructorsPage() {
  ...
  const params = useMemo(() => ({ search: search || undefined, status: status === 'All statuses' ? undefined : status, sub_department: department === 'All departments' ? undefined : department }), [search, status, department]);
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <div className="mb-5 flex flex-col gap-3 rounded-xl border border-border bg-card p-3 shadow-xs lg:flex-row lg:items-center">
      ...
      {(search || status !== 'All statuses' || department !== 'All departments') && <button type="button" data-testid="button-clear-filters" onClick={() => { setSearch(''); setStatus('All statuses'); setDepartment('All departments'); }} className="inline-flex h-10 items-center gap-1 rounded-lg px-2.5 text-[12px] font-bold text-muted-foreground hover:bg-secondary hover:text-foreground"><X size={14} /> Clear</button>}
    </div>
...
function InstructorRow({ instructor }: { instructor: Instructor }) {
  ...
  return <Link href={`/instructors/${instructor.id}`} data-testid={`link-instructor-${instructor.id}`} className="group grid gap-3 border-b border-border/70 px-4 py-4 transition-colors last:border-0 hover:bg-[#f8fafb] lg:grid-cols-[minmax(230px,1.6fr)_minmax(120px,.8fr)_minmax(140px,1fr)_minmax(120px,.8fr)_minmax(120px,.75fr)_40px] lg:items-center lg:gap-4 lg:px-5">
    ...
    <div><span data-testid={`status-instructor-${instructor.id}`} className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.06em] ${statusStyle(status)}`}>{status}</span></div>
